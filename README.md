...
WebGet is a single self-contained Python 3 script. It shells out to `dig`/`host`
...
```bash
git clone https://github.com/<you>/webget.git
cd webget
chmod +x install.sh
./install.sh
```
...
completely unrelated tool that will not work here. WebGet wants ProjectDiscovery's
...
If none is found, WebGet falls back to a built-in raw-socket prober — usable, but
...
```bash
# typical run
python3 webget.py -i targets.txt -s subs.txt -O out/ --resolve

# interactive: prompts for folders and picks files for you
python3 webget.py

# correlation only, skip probing
python3 webget.py -i targets.txt -s subs.txt -O out/ --no-probe

# offline: classify a previously captured httpx -json file
python3 webget.py -i targets.txt -s subs.txt -O out/ --json-in httpx.jsonl
```
...
Run `python3 webget.py --help` for the full list.
...
per client IP, and the failure mode is *bursts*, not total volume. WebGet paces
...
WebGet is for authorized security testing only. You are responsible for ensuring
