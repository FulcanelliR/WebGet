...
urls.txt policy (this is the important part):
    ...
    python3 webget.py -i ips.txt -s subs.txt -O out/ --resolve
    python3 webget.py                      # interactive folder prompts
    python3 webget.py -i ips.txt -s subs.txt -O out/ --no-probe
    python3 webget.py ... --json-in httpx.jsonl   # offline: parse captured httpx json
...
# Standard web ports only. webget's job is base URLs (https://host, http://host);
...
def probe_with_httpx(list_path, ports, threads, timeout, retries, rate, binary):
    if not os.path.isfile(list_path):
        raise RuntimeError(f"FQDN list not found at {list_path} (webget should have "
...
# ===== from cli.py =====
"""webget CLI: correlate an IP list with subfinder output, then probe FQDNs to URLs."""
OUTPUT_NAMES = {
    "results": "results.tsv",
    "urls": "urls.txt",
...
def interactive():
    print(f"=== WebGet {__version__} interactive setup ===")
    while True:
    ...
    return ns
def build_parser():
    p = argparse.ArgumentParser(prog="webget",
