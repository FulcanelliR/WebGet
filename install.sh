#!/usr/bin/env bash
#
# install.sh - dependency setup for reconx
#
# reconx itself is a single self-contained Python 3 script with NO third-party
...
# reconx looks for the real Go tool at ~/go/bin/httpx, then a verified 'httpx'
...
is_pd_httpx() {
  ...
  # library errors on the flag. This mirrors reconx's own detection.
...
if [ "$HTTPX_OK" -eq 0 ]; then
  ...
  if command -v go >/dev/null 2>&1; then
    ...
    if [ -x "$HOME/go/bin/httpx" ] && is_pd_httpx "$HOME/go/bin/httpx"; then
      ...
      case ":$PATH:" in
        *":$HOME/go/bin:"*) : ;;
        *) warn "Add Go bin to your PATH so reconx (and you) can find it:"
    ...
    err "reconx will fall back to its built-in socket prober if httpx is absent,"
...
if command -v python3 >/dev/null 2>&1; then
  ...
  ok "python3 found: $PYV (reconx needs 3.6+; no pip packages required)"
else
  err "python3 not found. Install Python 3.6 or newer."
fi

echo
ok "setup check complete. Run reconx with:  python3 reconx.py -i ips.txt -s subs.txt -O out/ --resolve"
echo "   (or just 'python3 reconx.py' with no args for interactive mode)"
